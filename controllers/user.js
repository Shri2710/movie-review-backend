...
exports.forgotPassword = async (req, res) => {
  ...
  if (resetToken)
    return sendError(res, "You can request OTP only after one hour");
...
exports.resetPassword = async (req, res) => {
  ...
  res.json({
    message:
      "Password Reset is successfully done, You can now use use new password",
  });
};
...
exports.signin = async (req, res) => {
  ...
  const match = await user.compareToken(password);

  const { _id, name, userEmail } = user;

  if (!match) return sendError(res, "Email / password mismatch");
  ...
  res.json({ id: _id, name, userEmail, token: jwtToken });
  }catch(error){
